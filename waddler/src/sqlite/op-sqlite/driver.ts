...
const createSqlTemplate = (
	...
	const fn = <T>(strings: TemplateStringsArray, ...params: SQLParamType[]): OpSqliteSQLTemplate<T> => {
		...
		return new OpSqliteSQLTemplate<T>(sql, client, dialect, { logger });
	};
	...
	Object.assign(fn, {
		...
		unsafe: (
			...
			const unsafeDriver = new OpSqliteSQLTemplate(sql, client, dialect, { logger }, options);
...
export function waddler({ client, config }: { client: DB; config?: WaddlerConfig }) {
