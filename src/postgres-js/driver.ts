...
const createSqlTemplate = (
	...
	const fn = <T>(strings: TemplateStringsArray, ...params: SQLParamType[]): PostgresSQLTemplate<T> => {
		...
		return new PostgresSQLTemplate<T>(query.query, query.params, client, dialect, query.queryChunks);
	};
	...
	Object.assign(fn, {
		...
		unsafe: async (
			...
			const unsafeDriver = new PostgresSQLTemplate(query, params, client, dialect, [], options);
