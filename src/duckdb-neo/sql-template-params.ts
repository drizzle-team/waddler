...
export type DuckdbNeoIdentifier =
	| string
	| string[]
...
export class DuckdbNeoSQLIdentifier extends SQLIdentifier {
	constructor(private readonly value: DuckdbNeoIdentifier) {
		super();
	}
	...
	static objectToSQL(object: {
		...
		DuckdbNeoSQLIdentifier.checkObject(object);
	...
	generateSQL() {
		...
		if (Array.isArray(this.value)) {
			...
			if (
				...
				return {
					sql: `${
						this.value
							.map((element) => DuckdbNeoSQLIdentifier.objectToSQL(element))
							.join(', ')
					}`,
				};
			}
		...
		if (typeof this.value === 'object' && this.value !== null) {
			...
			return { sql: DuckdbNeoSQLIdentifier.objectToSQL(this.value) };
		}
...
export class DuckdbNeoSQLDefault extends SQLDefault {
	generateSQL() {
		return { sql: 'default' };
	}
}

export type Value = string | number | bigint | boolean | Date | DuckdbNeoSQLDefault | null | Value[];
export type DuckdbNeoValues = Value[][];
export class DuckdbNeoSQLValues extends SQLValues {
	constructor(private readonly value: DuckdbNeoValues) {
		super();
	}
	...
	generateSQL() {
		...
		return {
			sql: this.value
				.map((val) => DuckdbNeoSQLValues.arrayToSQL(val))
				.join(', '),
		};
	}
	...
	private static arrayToSQL(array: Value[]) {
		if (Array.isArray(array)) {
			...
			return `(${array.map((val) => DuckdbNeoSQLValues.valueToSQL(val)).join(', ')})`;
		}
	...
	private static valueToSQL(value: Value): string {
		...
		if (value instanceof DuckdbNeoSQLDefault) {
		...
		if (Array.isArray(value)) {
			return `[${value.map((arrayValue) => DuckdbNeoSQLValues.valueToSQL(arrayValue))}]`;
		}
